...
test('edit_file replaces a unique match', async () => {
  ...
  try {
    ...
    const result = await tools.edit_file.execute!(
      { path: file, edits: [{ old_string: 'world', new_string: 'mars' }] },
      ctx,
    );
  ...
  try {
    ...
    await expect(
      tools.edit_file.execute!(
        { path: file, edits: [{ old_string: 'xyz', new_string: 'abc' }] },
        ctx,
      ),
...
test('edit_file throws on ambiguous match without replace_all', async () => {
  ...
  try {
    ...
    await expect(
      tools.edit_file.execute!(
        { path: file, edits: [{ old_string: 'hello', new_string: 'hi' }] },
        ctx,
      ),
...
test('edit_file with replace_all replaces every occurrence', async () => {
  ...
  try {
    ...
    const result = await tools.edit_file.execute!(
      {
        path: file,
        edits: [{ old_string: 'hello', new_string: 'hi', replace_all: true }],
      },
      ctx,
    );
...
test('edit_file throws when old_string equals new_string', async () => {
  ...
  try {
    ...
    await expect(
      tools.edit_file.execute!(
        { path: file, edits: [{ old_string: 'hello', new_string: 'hello' }] },
        ctx,
      ),
...
test('edit_file rejects empty old_string', async () => {
  ...
  try {
    ...
    await expect(
      tools.edit_file.execute!(
        { path: file, edits: [{ old_string: '', new_string: 'x' }] },
        ctx,
      ),
    ).rejects.toThrow();
  } finally {
...
test('edit_file treats new_string $-patterns as literal text', async () => {
  ...
  try {
    ...
    await tools.edit_file.execute!(
      { path: file, edits: [{ old_string: 'BAR', new_string: '$& $1 $$' }] },
      ctx,
    );
...
test('edit_file applies multiple edits sequentially', async () => {
  const dir = await makeTempDir();
  try {
    const file = join(dir, 'multi.txt');
    await writeFile(file, 'apple banana cherry');
    const result = await tools.edit_file.execute!(
      {
        path: file,
        edits: [
          { old_string: 'apple', new_string: 'ALPHA' },
          { old_string: 'banana', new_string: 'BETA' },
          { old_string: 'cherry', new_string: 'GAMMA' },
        ],
      },
      ctx,
    );
    ...
    expect(result).toContain('3 replacement');
    expect(await Bun.file(file).text()).toBe('ALPHA BETA GAMMA');
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});

test('edit_file second edit can target text introduced by the first', async () => {
  const dir = await makeTempDir();
  try {
    ...
    expect(await Bun.file(file).text()).toBe('three');
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
...
test('edit_file rejects empty edits array', async () => {
  ...
  try {
    ...
    await expect(
      tools.edit_file.execute!({ path: file, edits: [] }, ctx),
    ).rejects.toThrow();
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
